...
function renderWeatherData(weatherData) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    let html = `<p class="${i}title"><b>${date}</b></p>`;
    ...
    html = `<p class="${i}day">Max ${weather} &deg C</p>`;
    selectColumn.insertAdjacentHTML('beforeend', html);

    //render max temp
    weather = weatherData.daily[`${i}`].weather[0].main;
    selectColumn = document.querySelector(`.a${i}col`);
    html = `<p class="${i}day">${weather}</p>`;
    selectColumn.insertAdjacentHTML('beforeend', html);

    //render max temp
    weather = weatherData.daily[`${i}`].weather[0].icon;
    selectColumn = document.querySelector(`.a${i}col`);
    html = `<p class="${i}day"><img src="http://openweathermap.org/img/wn/${weather}@2x.png"></p>`;
    selectColumn.insertAdjacentHTML('beforeend', html);
